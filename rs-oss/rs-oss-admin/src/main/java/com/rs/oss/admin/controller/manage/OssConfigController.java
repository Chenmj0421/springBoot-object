 ...
 */
@Controller
@RequestMapping("/manage/ossConfig")
@Api(value = "OssConfig管理", description = "OssConfig管理")
public class OssConfigController extends BaseController {

    private static Logger log = LoggerFactory.getLogger(OssConfigController.class);
    ...
    @RequiresPermissions("oss:config:read")
    @ApiImplicitParams({
    ...
    public Object list(
        ...
        OssConfigExample ossConfigExample = new OssConfigExample();
        if (!StringUtils.isBlank(sort) && !StringUtils.isBlank(order)) {
            ossConfigExample.setOrderByClause(sort + " " + order);
        }
        long total = ossConfigService.countByExample(ossConfigExample);
        ...
        List<OssConfig> rows = ossConfigService.selectByExampleForOffsetPage(ossConfigExample, offset, limit);
        ...
        @RequiresPermissions("oss:config:create")
        @RequestMapping(value = "/create", method = RequestMethod.POST)
        @ResponseBody
        public Object create(@ModelAttribute OssConfig ossConfig) {
            int count = ossConfigService.insertSelective(ossConfig);
            return BaseResult.newSuccess();
        }

        @ApiOperation(value = "删除")
        @RequiresPermissions("oss:config:delete")
        @ApiImplicitParams({
        ...
        public Object delete(@PathVariable("ids") String ids) {
            int count = ossConfigService.deleteByPrimaryKeys(ids);
            return BaseResult.newSuccess();
        }
        ...
        @ResponseBody
        public Object update(@PathVariable("id") String id, @ModelAttribute OssConfig ossConfig) {
            int count = ossConfigService.updateByPrimaryKeySelective(ossConfig);
            return BaseResult.newSuccess();
        }
        ...
        public Object info(@PathVariable("id") String id, ModelMap modelMap) {
            OssConfig ossConfig = ossConfigService.selectByPrimaryKey(id);
            modelMap.put("ossConfig", ossConfig);
