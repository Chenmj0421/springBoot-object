...
public class HttpHelper {
     ...
     * @throws IOException
     */
    public static HttpResult executeUploadFile(CloseableHttpClient httpClient, String remoteFileUrl, File file, String charset, boolean closeHttpClient) throws IOException {
        CloseableHttpResponse httpResponse = null;
        try {
            if (httpClient == null) {
                httpClient = createHttpClient();
     ...
     * @throws IOException
     */
    public static Object executeUploadFileStream(String remoteFileUrl, MultipartFile file) throws ClientProtocolException, IOException {
        return executeUploadFileStream(null, remoteFileUrl, file, null, DEFAULT_CHARSET, true);
    }

    public static boolean uploadFileStreamToHadoop(String url, MultipartFile file, String ip, String fileName) throws IOException {
        return uploadFileStreamToHadoop(null, url, file, ip, fileName, DEFAULT_CHARSET, true);
    }

    public static String getFilePathFromHadoop(String url, String ip) throws IOException {
        return getFilePathFromHadoop(ip, null, url, DEFAULT_CHARSET, true);
     ...
     * @throws IOException
     */
    public static String getFilePathFromHadoop(String ip, CloseableHttpClient httpClient, String remoteFileUrl, String charset, boolean closeHttpClient) throws ClientProtocolException, IOException {
        CloseableHttpResponse httpResponse = null;
        try {
            if (httpClient == null) {
                httpClient = createHttpClient();
            ...
            if (HttpStatus.SC_TEMPORARY_REDIRECT == httpResponse.getStatusLine().getStatusCode()) {
                String location = null;
                Header locationHeader = httpResponse.getFirstHeader("Location");
                if (locationHeader != null) {
                    location = locationHeader.getValue().replaceAll("localhost", ip).replaceAll("127.0.0.1", ip);
                    return location;
                }
     ...
     * @throws IOException
     */
    public static boolean uploadFileStreamToHadoop(CloseableHttpClient httpClient, String remoteFileUrl, MultipartFile file, String ip, String fileName, String charset, boolean closeHttpClient) throws ClientProtocolException, IOException {
        CloseableHttpResponse httpResponse = null;
        try {
            if (httpClient == null) {
                httpClient = createHttpClient();
            ...
            HttpPut httpPut = new HttpPut(remoteFileUrl);
            // 设置 httpentity 实体
            httpPut.setEntity(httpEntity);
            httpResponse = httpClient.execute(httpPut);
            if (HttpStatus.SC_TEMPORARY_REDIRECT == httpResponse.getStatusLine().getStatusCode()) {
                return createFileStreamToHadoop(httpResponse, httpClient, file, ip, fileName, charset, true);
    ...
    public static boolean createFileStreamToHadoop(HttpResponse httpResponse, CloseableHttpClient httpClient, MultipartFile file, String ip, String fileName, String charset, boolean closeHttpClient) throws ClientProtocolException, IOException {
        CloseableHttpResponse httpResponses = null;
        try {
            if (httpClient == null) {
                httpClient = createHttpClient();
            }
     ...
     * @throws IOException
     */
    public static void uploadFile(String webhdfs, MultipartFile file, String method) throws IOException {
        HttpURLConnection con;
        con = getConnection(webhdfs, method);
     ...
     * @return
     */
    public static HttpURLConnection getConnection(String strurl, String method) throws IOException {
        URL url = null;
     ...
     * @throws IOException
     */
    public static Object executeUploadFileStream(String remoteFileUrl, MultipartFile file, String userId) throws ClientProtocolException, IOException {
        return executeUploadFileStream(null, remoteFileUrl, file, userId, DEFAULT_CHARSET, true);
    }

    public static Object uploadFileStreamToHadoop(String ip, String url, MultipartFile file, String fileName) throws IOException {
        return uploadFileStreamToHadoop(null, url, file, ip, fileName, DEFAULT_CHARSET, true);
    }
     ...
     * @throws IOException
     */
    public static Object executeUploadFileStream(CloseableHttpClient httpClient, String remoteFileUrl, MultipartFile file, String userId, String charset, boolean closeHttpClient) throws ClientProtocolException, IOException {
        CloseableHttpResponse httpResponse = null;
        try {
            if (httpClient == null) {
                httpClient = createHttpClient();
            ...
            StringBody index = new StringBody(userId, ContentType.create("text/plain", Consts.UTF_8));
            // 创建 part contentBody,第一个是值，第二个参数是类型，第三个参数是编码格式
            // 创建MultipartEntityBuilder对象
            MultipartEntityBuilder builder = MultipartEntityBuilder.create();
            // 设置编码
            ...
            builder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
            // 添加二进制文件流，第一个参数是接口参数名，第二个参数是文件的二进制流，第三个参数是文件名
            //builder.addBinaryBody("files", file.getInputStream(), ContentType.create("multipart/form-data"), fileName);
            // 添加参数和参数值
            builder.addPart("userId", index);
            // 创建 httpentity 实体
            HttpEntity httpEntity = builder.build();
            HttpPut httpPost = new HttpPut(remoteFileUrl);
            // 设置 httpentity 实体
            httpPost.setEntity(httpEntity);
            httpResponse = httpClient.execute(httpPost);
            System.out.print(getResult(httpResponse, charset));
            if (HttpStatus.SC_OK == httpResponse.getStatusLine().getStatusCode()) {
                return JSONObject.fromObject(getResult(httpResponse, charset));
            } else {
                return null;
            }
        } finally {
